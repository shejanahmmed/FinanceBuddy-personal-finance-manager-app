...
object DatabaseMigrations {
    ...
    // ─────────────────────────────────────────────────────────
    // v17 → v18 : Add imageUri column to payees table
    // ─────────────────────────────────────────────────────────
    val MIGRATION_17_18 = object : Migration(17, 18) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                "ALTER TABLE `payees` ADD COLUMN `imageUri` TEXT DEFAULT NULL"
            )
        }
    }

    // ─────────────────────────────────────────────────────────
    // v18 → v19 : Add monthYear column to budgets table
    // ─────────────────────────────────────────────────────────
    val MIGRATION_18_19 = object : Migration(18, 19) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `budgets` ADD COLUMN `monthYear` TEXT NOT NULL DEFAULT ''")
            val currentMonth = java.text.SimpleDateFormat("yyyy-MM", java.util.Locale.US).format(java.util.Date())
            db.execSQL("UPDATE `budgets` SET `monthYear` = '$currentMonth' WHERE `monthYear` = ''")
        }
    }
